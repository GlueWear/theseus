...
function onChannel(raw) {
  ...
  for (const msg of Array.isArray(parsed) ? parsed : [parsed]) {
    // Track channel event ids so we can ack; Eyre may encode them as numbers
    // or decimal strings depending on the JSON path. Without acks it clogs.
...
function sendViaGateway(rec, from, bytes, reason = 'gateway') {
  ...
  if (!peer) { console.log(`[transport] OUT ~${from} no gateway for ${reason}, drop`); return false; }
  rec.udp.send(bytes, peer.port, peer.addr, (e) => {
